...
function editEmployee(tagId, Employeeno){
	...
	var selectedCrew = document.getElementById(CREW + tagId).textContent;
	
    document.getElementsByName(E_E_NO)[0].value = document.getElementById(E_NO + tagId).textContent;
    document.getElementsByName(E_E_NO)[1].value = document.getElementById(E_NO + tagId).textContent;
    ...
    var selectedCrewNo = document.getElementsByName(E_CREW)[0];
    for(var i=0; i< selectedCrewNo.length;i++){
    	if(selectedCrewNo[i].label == selectedCrew){
    		selectedCrewNo[i].selected = true;
    		break;
    	}
    }
